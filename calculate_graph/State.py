...
class State:
	...
	@classmethod
	def from_key(self):
		""" Define a state using three integers: top hash, bottom hash, red car position. All other state values will be derived."""
		pass
	...
	def isValid(self):
		"""Confirm values set for the state are consistent with the internal data model and the rules of the Rush Hour game"""
	...
	def set_derived_fields(self):
		...
		if self.isValid():
			self.board_as_bit_string = bin(self.board_top_hash)[2:].zfill(54) + bin(self.board_bottom_hash)[2:].zfill(54)
	...
	def __lt__(self,other):
		if type(other) is type(self):
			return self.red_car_end_a > other.red_car_end_a or \
				   (self.red_car_end_a == other.red_car_end_a and int(self.board_as_bit_string,2) < int(other.board_as_bit_string,2))
		return False
	
	def __eq__(self, other):
		if type(other) is type(self):
			return self.board_top_hash == other.board_top_hash and self.board_bottom_hash == other.board_bottom_hash and self.red_car_end_a == other.red_car_end_a
		return False
	...
	def neighbor_from_move_car_left(car_end_a):
		"""Create a new state derived from the current state by moving the indicated car left one space. Returns an instance of State."""
	...
	def set_array_of_pieces(self):
		...
		for i in range(36):
			if board[i] == self.VERTICAL_CAR:
				self.pieces.append( Piece( end_a = i, end_b = i+6 , topology = self.VERTICAL_CAR,symbol=None,color=None ) )
				board[i] = self.BLANK_SPACE
				board[i+6] = self.BLANK_SPACE
			
			elif board[i] == self.HORIZONTAL_CAR:
				self.pieces.append( Piece( end_a = i, end_b = i+1, topology = self.HORIZONTAL_CAR,symbol = None, color = None) )
				board[i] = self.BLANK_SPACE
				board[i+1] = self.BLANK_SPACE
			
			elif board[i] == self.VERTICAL_TRUCK:
				self.pieces.append( Piece( end_a =i, end_b = i + 12, topology = self.VERTICAL_TRUCK,symbol = None, color = None) )
				board[i] = self.BLANK_SPACE
				board[i+6] = self.BLANK_SPACE
				board[i+12] = self.BLANK_SPACE
			
			elif board[i] == self.HORIZONTAL_TRUCK:
				self.pieces.append( Piece(end_a = i, end_b = i+2, topology = self.HORIZONTAL_TRUCK,symbol = None, color = None) )
				board[i] = self.BLANK_SPACE
				board[i+1] = self.BLANK_SPACE
	...
	def can_move_left(self,piece):
		"""Moving a piece left requires piece to not be on left edge and apace to left of piece must be unoccupied"""
		if piece.end_a % 6 == 0 or piece.topology not in [self.HORIZONTAL_CAR, self.HORIZONTAL_TRUCK]:
			return False
		return self.board[piece.end_a - 1] == self.BLANK_SPACE
	
	def can_move_right(self,piece):
		"""Moving a piece right requires piece to not be on right edge and space to right of piece must be unoccupied"""
		if piece.end_b % 6 == 5 or piece.topology not in [self.HORIZONTAL_CAR, self.HORIZONTAL_TRUCK]:
			return False
		return self.board[ piece.end_b + 1] == self.BLANK_SPACE
	
	def can_move_up(self,piece):
		"""Moving a piece up requires piece to not be on top edge and space above piece must be unoccupied"""
		if piece.end_a in range(6) or piece.topology not in [self.VERTICAL_CAR, self.VERTICAL_TRUCK]:
			return False
		return self.board[ piece.end_a - 6] == self.BLANK_SPACE
	
	def can_move_down(self,piece):
		"""Moving a piece down requires piece to not be on bottom edge and space below piece must be unoccupied"""
		if piece.end_b in range(30,36) or piece.topology not in [self.VERTICAL_CAR, self.VERTICAL_TRUCK]:
			return False
	...
	def derive_state_by_moving_piece(self,piece,direction):
		...
		nbr_state_dict = {'red_car_end_a':red_car_end_a, 'board_top_hash':board_top_hash, 'board_bottom_hash':board_bottom_hash}
		nbr_state = State(**nbr_state_dict)
		return nbr_state
			
		
	def derive_neighbors(self):
		""" Derives neighbors of this State based on the rules of Rush Hour. This method returns an array of States."""
	...
	def sort_pieces_topologically(self):
		...
		v_cars = sorted([p for p in self.pieces if p.topology == self.VERTICAL_CAR],key=lambda p: p.end_a)
		v_trucks = sorted([p for p in self.pieces if p.topology == self.VERTICAL_TRUCK ],key=lambda p: p.end_a)
		h_cars = sorted([p for p in self.pieces if p.topology == self.HORIZONTAL_CAR],key=lambda p: p.end_a)
		h_trucks = sorted([p for p in self.pieces if p.topology == self.HORIZONTAL_TRUCK],key=lambda p: p.end_a)
	...
	def color_and_label_pieces_topologically(self):
		...
		for piece in self.pieces:
			if piece.end_a == self.red_car_end_a:
				colored_pieces.append( Piece( end_a = piece.end_a, end_b = piece.end_b, topology = piece.topology,symbol=self.RED_SYMBOL,color= self.RED_COLOR ) )
				
			elif piece.topology in car_topologies:
				colored_pieces.append( Piece( end_a = piece.end_a, end_b = piece.end_b, topology = piece.topology,symbol=self.car_symbols[car_index],color = self.car_colors[car_index] ) )
				car_index = car_index + 1
				
			else:
				colored_pieces.append( Piece( end_a = piece.end_a, end_b = piece.end_b, topology = piece.topology,symbol=self.truck_symbols[truck_index],color = self.truck_colors[truck_index] ) )
	...
	def svg(self):
		...
		#svg = '<?xml version="1.0"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">'
		svg = '<svg width="'+str(board_size)+'" height="'+str(board_size)+'" xmlns="http://www.w3.org/2000/svg">'
		
		# Board Outline
		svg = svg + """<rect x="0" y="0" width = "%d" height = "%d" style="fill:#%s; stroke:black; stroke-width:2; stroke-opacity:0.5;"/> """%(board_size,board_size,self.BLANK_COLOR)
		...
		svg = svg + '<defs>'		
		svg = svg + '    <g id="car" >'
		svg = svg + '        <rect x="0" y="0" height="' + str(car_outer_height) + '" width="' + str(car_outer_width) + '" style="opacity:0.0"/>'
		svg = svg + '        <rect x="' + str(car_inner_x)+ '" y="' + str(car_inner_y) + '" ' 
		svg = svg + 'height ="' + str(car_inner_height) + '" width="'+ str(car_inner_width) + '" rx="5" ry="5"/>'
		svg = svg + '    </g>'
		
		svg = svg + '    <g id="truck" >'
		svg = svg + '        <rect x="0" y="0" height="' + str(truck_outer_height) + '" width="' + str(truck_outer_width) + '" style="opacity:0.0"/>'
		svg = svg + '        <rect x="' + str(truck_inner_x)+ '" y="' + str(truck_inner_y) + '" ' 
		svg = svg + 'height ="' + str(truck_inner_height) + '" width="'+ str(truck_inner_width) + '" rx="5" ry="5"/>'
		svg = svg + '    </g>'		
		svg = svg + '</defs>'
		...
		# draw grid lines
		for i in range(1,6):
			svg = svg + '<line x1="0" y1="%d" x2="%d" y2="%d" style="stroke: black;"/>'%(i*grid_size,board_size,i*grid_size)
			svg = svg + '<line x1="%d" y1="0" x2="%d" y2="%d" style="stroke: black;"/>'%(i*grid_size,i*grid_size,board_size)
		...
		for piece in self.pieces:
			...
			text_style = ' text-anchor="middle" alignment-baseline="central" style="font-weight:bold" '
			xform_rotate = 'transform="rotate(90,' + str(rot_x)+','+str(rot_y)+')"' 
			if piece.topology == self.VERTICAL_CAR:	
				svg = svg + '<use xlink:href="#car" x="'+ str(x) +'" y="'+ str(y) + '"' + xform_rotate + ' style="fill:#'+color+'" ;" />' 
			...
			if piece.topology == self.VERTICAL_TRUCK:
				svg = svg + '<use xlink:href="#truck" x="'+ str(x) +'" y="'+str(y)+ '"' +  xform_rotate +' style="fill:#'+color+'" ;" />' 
				text_x = ' x="' + str(x + truck_outer_height / 2) + '"'
				text_y = ' y="' + str(y + truck_outer_width / 2)  + '"'
				svg = svg + '<text ' + text_x + text_y + text_style + '>' + text_symbol + '</text>' 		
			if piece.topology == self.HORIZONTAL_CAR:
			
				svg = svg + '<use xlink:href="#car" x="'+ str(x) +'" y="'+str(y)+'" style="fill:#'+color+'" ;" />' 
		...
		svg = svg + '<use xlink:href="#car" x="'+ str(red_x) +'" y="'+str(red_y)+'" style="fill:#'+ self.RED_COLOR +'" ;" />'
	...
	# !!!! TODO - Add inputs to allow color scheme to be passed in (car_colors = [ . . . ], truck_colors = [ . . .] )
...
state_dict = {'board_top_hash':7599893090940448,'board_bottom_hash':2200105390080,'red_car_end_a':13}	
s = State(**state_dict)
