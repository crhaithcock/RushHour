...
declare @temp_transitions as table(
	pre_tran_id bigint,
    post_tran_id bigint,
    pre_piece_id bigint,
...
/* All we need to compute is  x ~ y  */
insert into puzzle_state_transitions
select pre_tran_id,post_tran_id,pre_piece_id,post_piece_id

go

/* We get y ~ x for free */
insert into puzzle_state_transitions
select post_tran_id,pre_tran_id,post_piece_id,pre_piece_id

go

*/

insert into @temp_transitions
    ...
    from
            ...
            /*  The goal - a set of all possible transitions from the exisitng states
            
                Start with a superset of the goal and winnow down the set to our goal.
                One could start with the set of all state pairs. We take a different
                approach.

                Start with pairing states if they are in the same combinatorial class.

                Second, x ~ y => y~ x. So, we only need only consider pairs
                where pre state id < post state id
            */
            ...
            piece_placements_puzzle_states pre_transition_pieces -- get all pieces for pre transition puzzle state
            on pre_transition_state_id = pre_transition_pieces.puzzle_state_id

            left join
            piece_placements_puzzle_states post_transition_pieces -- get all pieces for post transition puzzle state
